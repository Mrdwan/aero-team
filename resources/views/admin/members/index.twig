...
@section('content')
    <h2>
        ...
        <a href="{{ route('aero_team.admin.create') }}" class="btn btn-secondary">
            @include('admin::icons.add') New Team member
        </a>
    </h2>
    @include('admin::partials.alerts')
    <bulk-actions
        class="card p-0 block"
        :total-rows="{{ $teamMembers->total() }}"
        :ids="@json($teamMembers->modelKeys())"
        :has-more-posts="{{ $teamMembers->hasPages() ? 'true' : 'false' }}"
        v-slot="bulk"
    >
        <table>
            @include('admin::partials.bulk-header', ['list' => $teamMembers])
